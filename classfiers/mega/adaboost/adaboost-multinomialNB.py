...
# 创建 GridSearchCV 对象
rf_grid_search = GridSearchCV(base_classifier, param_grid_multonomial, scoring=scoring, cv=5, refit='accuracy')

# 执行随机森林的网格搜索
rf_grid_search.fit(X, y)

# 输出随机森林最佳超参数组合
print("MultinomialNB Best Parameters:", rf_grid_search.best_params_)
...
best_base_classifier = MultinomialNB(**rf_grid_search.best_params_)
# 创建 AdaBoost 分类器
...
# 将结果保存到Excel文件
results_df.to_excel('mega-adaboost-randomForest-results.xlsx', index=False)