...
def evaluate_model(model, X, y):
    ...
    y_pred_binary = (y_pred >= 0.5).astype(int)  # Convert probability values to binary classes
    precision = precision_score(y, y_pred_binary)
    recall = recall_score(y, y_pred_binary)
    f1 = f1_score(y, y_pred_binary)
    auc = roc_auc_score(y, y_pred)  # No need to specify average and multi_class parameters
...
output.to_excel('bloated_elm_lin_results.xlsx', index=False)